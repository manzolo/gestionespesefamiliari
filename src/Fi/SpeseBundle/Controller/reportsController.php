...
class reportsController extends Controller {
    ...
    public function excelspeseAction(Request $request) {
        ...
        /* @var $qb \Doctrine\ORM\QueryBuilder */
        $qb = $em->createQueryBuilder("reports");
        $qb->select("m utenteid, u.nome nomeutente, u.cognome cognomeutente, YEAR(m.data) anno, SUM(m.importo) as importototale")
                ->from("FiSpeseBundle:movimento", 'm')
                ->leftJoin("FiSpeseBundle:utente", 'u', 'WITH', '(m.utente_id = u.id)')
                ->leftJoin("FiSpeseBundle:tipologia", 't', 'WITH', '(m.tipologia_id = t.id)')
                ->leftJoin("FiSpeseBundle:categoria", 'c', 'WITH', '(t.categoria_id = c.id)')
                //->andWhere('u.id = :utenteid')
                ->groupBy('m.utente_id, u.nome, u.cognome, anno')
        ...
        /* @var $qb \Doctrine\ORM\QueryBuilder */
        $qb = $em->createQueryBuilder("reports");
        $qb->select("m utenteid, f.descrizione descrizionefamiglia, u.nome nomeutente, u.cognome cognomeutente, c.descrizione descrizionecategoria, YEAR(m.data) anno, SUM(m.importo) as importototale")
                ->from("FiSpeseBundle:movimento", 'm')
                ->leftJoin("FiSpeseBundle:utente", 'u', 'WITH', '(m.utente_id = u.id)')
                ...
                ->leftJoin("FiSpeseBundle:categoria", 'c', 'WITH', '(t.categoria_id = c.id)')
                //->andWhere('u.id = :utenteid')
                ->groupBy('m.utente_id, f.descrizione, u.nome, u.cognome,  c.descrizione , anno')
        ...
        /* @var $qb \Doctrine\ORM\QueryBuilder */
        $qb = $em->createQueryBuilder("reports");
        $qb->select("m utenteid, f.descrizione descrizionefamiglia, u.nome nomeutente, u.cognome cognomeutente, c.descrizione descrizionecategoria, t.descrizione descrizionetipologia, YEAR(m.data) anno, SUM(m.importo) as importototale")
                ->from("FiSpeseBundle:movimento", 'm')
                ->leftJoin("FiSpeseBundle:utente", 'u', 'WITH', '(m.utente_id = u.id)')
                ->leftJoin("FiSpeseBundle:famiglia", 'f', 'WITH', '(u.famiglia_id = f.id)')
                ->leftJoin("FiSpeseBundle:tipologia", 't', 'WITH', '(m.tipologia_id = t.id)')
                ->leftJoin("FiSpeseBundle:categoria", 'c', 'WITH', '(t.categoria_id = c.id)')
                //->andWhere('u.id = :utenteid')
                ->groupBy('m.utente_id, f.descrizione, u.nome, u.cognome,  c.descrizione , t.descrizione, anno')
    ...
    public function writeReportTotale($resultset, $sheet) {
        ...
        foreach ($resultset as $record) {
            ...
            $col = $col + 1;
            $sheet->setCellValueByColumnAndRow($col, $row, $record["importototale"]);
            $col = $col + 1;
            $row++;
        }
    }
